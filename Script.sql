...
create table Sucursal(idSucursal int primary key not null, idGerente varchar(45), dir varchar(45), nombre varchar(45), tel int, foreign key(idGerente) references Gerente(rfc) on delete cascade on update cascade);
...
create table Venta(idVenta int primary key not null, idCajero varchar(45), idSocio varchar(45), fecha date, importe float, foreign key(idCajero) references Cajero(rfc) on delete cascade on update cascade, foreign key(idSocio) references Socio(curp) on delete cascade on update cascade);
...
insert into Sucursal values(2, 'OERA971231', 'Calle 11 #2', 'Lomas Estrella', 10203040);
